...
const SelectOptionTag = ({ option, active = false, ...props }: TagProps) => {
  ...
  return (
    <span className={`${styles.tag} ${active ? styles.active : ''}`} {...props}>
  ...
  return (
    <>
      <Space size={8}>
        {options.map(item => (
          <SelectOptionTag
            active={(typeof item === 'object' ? item.value : item) === checkValue}
            option={item}
            onClick={() => handleChange(typeof item === 'object' ? item.value : item)}
